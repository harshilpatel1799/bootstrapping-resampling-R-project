#########################################################
# FINAL PROJECT: PART 3.Data Set 2 - STAT 3201(DONGES)
...
#Project Data Set 2.csv gives 250 observations of the time spent studying for an exam of students. Use this data to take 1,000 resamples (i.e.,
#sample with replacement) of size 250 from the observed data. Plot the empirical distribution of the sample mean, estimate the mean of the sample mean, 
...
#percentiles. Compare and contrast your results for the two sample sizes from Project Data Set 1.csv.

#EXTRACT DATA FROM DATA SET FILE(.CSV)
dataset1<-read.csv("Project Data 2.csv", header = TRUE)
studyTimeValues<- dataset1$x #vector of 250 observations of a student's time spent studying for exam.
...
for (i in 1:numberOfResamples) {
  samples<- sample(studyTimeValues,250, replace = TRUE)
...
hist(vectorOfSampleMeans,
     main = expression("Distribution of the Average Time Spent Studying for an Exam (Project Data Set 2)"),
     ylab='f(x)',
